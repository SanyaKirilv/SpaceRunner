...
public class PipeCreator : MonoBehaviour
{
    ...
    private IEnumerator StartSpawn()
    {
        ...
        while (i < variants.Count)
        {
            ...
            StartCoroutine(SpawnPipe9(i));
            StartCoroutine(SpawnPipe10(i));
    ...
    private IEnumerator SpawnPipe1(int i)
    {
        ...
        for (int q = 0; q < 4; q++)
        {
            ...
            _pipe.GetComponent<Pipe>().obstacles[q].SetActive(false);

            _pipe.GetComponent<Pipe>().down[q+4].SetActive(false);
            _pipe.GetComponent<Pipe>().up[q+4].SetActive(false);
            _pipe.GetComponent<Pipe>().obstacles[q+4].SetActive(false);
            yield return new WaitForSeconds(.125f);
        }
    ...
    private IEnumerator SpawnPipe9(int i)
    {
        var _pipe = Instantiate(prefab, new Vector3(55 * i, 440, 0), Quaternion.identity);
        for (int q = 0; q < 4; q++)
        {
            _pipe.GetComponent<Pipe>().down[q].SetActive(variants[i][q] == '1');
            _pipe.GetComponent<Pipe>().up[q].SetActive(false);
            _pipe.GetComponent<Pipe>().obstacles[q].SetActive(variants[i][q] != '1');
    ...
    private IEnumerator SpawnPipe10(int i)
    {
        var _pipe = Instantiate(prefab, new Vector3(55 * i, 495, 0), Quaternion.identity);
        for (int q = 0; q < 4; q++)
        {
            _pipe.GetComponent<Pipe>().down[q].SetActive(variants[i][q] == '1');
            _pipe.GetComponent<Pipe>().up[q].SetActive(false);
            ...
            _pipe.GetComponent<Pipe>().down[q+4].SetActive(variants[i][q] == '1');
            _pipe.GetComponent<Pipe>().up[q+4].SetActive(false);
            _pipe.GetComponent<Pipe>().obstacles[q+4].SetActive(variants[i][q] != '1');
            yield return new WaitForSeconds(.125f);
        }
        _pipe.name = $"Section_{variants[i]}";
        Destroy(_pipe.GetComponent<Pipe>());
